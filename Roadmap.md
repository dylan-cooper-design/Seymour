...
- [x] **3.1** Create MilestoneGroup component. Collapsible section with label, chevron, expand/collapse state.
- [x] **3.2** Create MilestoneItem component. Selectable child item with active/inactive states.
- [x] **3.3** Create MilestoneTree component. Renders a list of MilestoneGroup components from mock data.
- [x] **3.4** Define mock data structure for milestones. Match the Goal, Milestones, Decisions, Actions hierarchy.
- [x] **3.5** Wire up selection state. Clicking a MilestoneItem sets it as active.
- [x] **3.6** Wire up PlatformSelector (Foundation). Selecting it shows the project overview state.
- [x] **3.7** Visual polish. Match Figma styling (text sizes, spacing, selected state highlight, indentation).
...
- [x] **4.1** Implement three-column layout. When a sidebar item is selected, the main area splits into detail panel (left) + chat (right). When Foundation is selected with no sub-item, return to two-column.
- [x] **4.2** Create DetailHeader component. Renders the title of the selected item.
- [x] **4.3** Create SectionBlock component. Label + body content (supports text and lists).
- [x] **4.4** Create BulletList component. Styled list within sections.
- [x] **4.5** Build the Foundation view. Composed of SectionBlocks showing Goal, Milestones summary, Working Context.
- [x] **4.6** Build a generic milestone item view. Composed of SectionBlocks showing Context, Decision, Rationale, Pros, Cons, Assumptions, Open Questions.
- [x] **4.7** Wire detail panel to sidebar selection. Selecting a nav item renders the appropriate view with mock data.
- [x] **4.8** Visual polish. Typography, spacing, section label styling (small caps gold labels).
...
- [x] **5.1** Define the app state shape. Selected item, project data, conversations per item.
- [x] **5.2** Implement state management (React context or lightweight store, keep it simple).
- [x] **5.3** Create a mock data file representing a full project (goal, milestones, decisions, chat histories).
- [x] **5.4** Wire all three panels to shared state. Sidebar selection drives detail + chat content.
- [x] **5.5** Scope chat to selected nav item. Switching items loads a different conversation.
- [x] **5.6** Implement empty state to active state transition (before goal is set vs. after).
...
- [x] **6.1** Define the API call structure. What gets sent to Claude (system prompt + conversation history + current context).
- [x] **6.2** Handle milestone generation. When user sets a goal, Claude generates the milestone tree and populates the sidebar.
- [x] **6.3** Handle detail panel updates. When decisions are made in chat, the detail panel content updates.
- [x] **6.4** Add streaming responses (Seymour's messages appear progressively).
...
- [x] **7.1** Loading states. Typing indicator for chat while streaming.
- [x] **7.2** Empty states. No milestones yet, no chat history for a new item.
- [x] **7.3** Error states across all panels (not just chat).
- [x] **7.4** Keyboard navigation. Focus rings on all sidebar buttons for keyboard users.
- [x] **7.5** Responsive behavior — decided: desktop-only for MVP.
- [x] **7.6** Scrolling behavior. Sidebar scrolls independently, detail panel scrolls, chat auto-scrolls to bottom.
...
**Phase 3 (Sidebar Nav):** Done
**Phase 4 (Detail Panel):** Done
**Phase 5 (State):** ✅ Done
**Phase 6 (AI Integration):** ✅ Done
**Phase 7 (Polish):** ✅ Done